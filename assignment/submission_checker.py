...
CONDITIONS_DICT = [
    {
        ...
        "has_classes": True,
        "imports": [
            {"module": "random", "submodules": [], "start": 0, "end": 1},
        ...
        "classes": [
            {
                ...
                "methods": [
                    ...
                    },
                    {
                        "function": "add_edge",
                        "args": {
                            "arg_names": ["self", "src", "dest", "weight"],
                            "has_non_constant_args": False,
                        },
                        "has_return": False,
                        "start": 33,
    ...
    },
    {
        ...
        "classes": [
            {
                "name": "GraphTSPSmallGraph",
                "methods": [
                    {
                        "function": "tsp_small_graph",
                        "args": {
                            "arg_names": ["self", "start_vertex"],
                            "has_non_constant_args": False,
                        },
                        "has_return": True,
                        "start": 3,
    ...
    },
    {
        ...
        "classes": [
            {
                "name": "GraphTSPLargeGraph",
                "methods": [
                    {
                        "function": "tsp_large_graph",
                        "args": {
                            "arg_names": ["self", "start"],
                            "has_non_constant_args": False,
                        },
                        "has_return": True,
                        "start": 3,
    ...
    },
    {
        ...
        "classes": [
            {
                "name": "GraphTSPMediumGraph",
                "methods": [
                    {
                        "function": "tsp_medium_graph",
                        "args": {
                            "arg_names": ["self", "start"],
                            "has_non_constant_args": False,
                        },
...
class GradedTree:
        ...
        if (
            len(
                [
                    x
                    for x in tree.body
                    if not isinstance(x, tuple(IMPORTANT_OBJECTS.values()))
                ]
            )
            != 0
        ):
    ...
    def get_imports(self):
        if not self.has_imports:
            return None
        all_imports = [
            x for x in self.objects if isinstance(x, IMPORTANT_OBJECTS["imports"])
        ]
        imports = []
    ...
    def get_functions(self):
        if not self.has_functions:
            return None
        all_functions = [
            x for x in self.objects if isinstance(x, IMPORTANT_OBJECTS["functions"])
        ]
        functions = []
        for function in all_functions:
            ...
            non_constant_args = [
                x for x in function.args.defaults if not isinstance(x, ast.Constant)
            ]
    ...
    def get_assigns(self):
        if not self.has_assigns:
            return None
        all_assigns = [
            x for x in self.objects if isinstance(x, IMPORTANT_OBJECTS["assigns"])
        ]
        assigns = []
    ...
    def get_classes(self):
        if not self.has_classes:
            return None
        all_classes = [
            x for x in self.objects if isinstance(x, IMPORTANT_OBJECTS["classes"])
        ]
        classes = []
        for class_ in all_classes:
            ...
            methods = [
                self._get_functions(x)
                for x in class_.body
                if isinstance(x, IMPORTANT_OBJECTS["functions"])
            ]
...
def get_graded_cells(nb, conditions_dict=None):
    ...
    if len(code_cells) == 0:
        return (
            False,
            colored(
                "No code cells found in your notebook. Please refresh your workspace",
                "red",
            ),
        )
    text_case_fails = colored(
        "You deleted one or more graded cell. Please refresh your workspace and copy your solution in the cells that the graded functions are initially. Autograder will fail as it won't find these solutions.\nTo refresh work workspace, follow the instructions in the coursera item called '(Optional) Downloading your Notebook and Refreshing your Workspace.'",
        "red",
    )
    ...
    if conditions_dict is not None:
        if len(graded_cells) < len(conditions_dict):
            for graded_cell in graded_cells:
                ...
                if (
                    (graded_tree.has_imports == True)
                    and (graded_tree.has_other_values == False)
                    and (graded_tree.has_classes == False)
                    and (graded_tree.has_functions == False)
                ):
                    has_initial_import_cell = True
            if not has_initial_import_cell:
                return False, colored(
                    "You deleted the initial import cell and moved it to another cell. This will break the autograder. Please refresh your workspace and paste your solution there.\nTo refresh work workspace, follow the instructions in the coursera item called '(Optional) Downloading your Notebook and Refreshing your Workspace.' ",
                    "red",
                )
            return False, text_case_fails
...
def parse_graded_cell(graded_cell: dict):
    if not isinstance(graded_cell, dict):
        raise Exception(
            "graded_cell must be a dictionary with the graded cell information"
        )
...
def cell_checker(tree, code, cell_number, conditions_dict):
    ...
    if (functions is None) and conditions_dict["has_functions"]:
        return False, colored(
            f"There is no function detected in graded cell number {cell_number}. Autograder will likely fail. Make sure you haven't moved the functions to another cell.",
            "red",
        )
    if (imports is None) and conditions_dict["has_imports"]:
        return False, colored(
            f"There is no import detected in graded cell number {cell_number}. Autograder will likely fail. Make sure you haven't moved the imports to another cell.",
            "red",
        )
    if (classes is None) and conditions_dict["has_classes"]:
        return False, colored(
            f"There is no class object detected in graded cell number {cell_number}. Autograder will likely fail. Make sure you haven't moved the classes to another cell.",
            "red",
        )

    if (imports is not None) and (not conditions_dict["has_imports"]):
        msg = colored(
            f"There is one or more import detected in graded cell number {cell_number} where it should have any. Autograder will likely fail. Do not add new imports in a cell apart from the ones in the beginning, or the ones that are already in the graded cell. Imports that should not exist in this cell:\n",
            "red",
        )
    ...
    if other_values is not None:
        msg = colored(
            f"There is more content in graded cell number {cell_number} than it should have. Autograder will likely fail. Please remove them before submitting. Content that must be removed or moved to the correct place:\n",
            "red",
        )
    ...
    if (functions is not None) and (not conditions_dict["has_functions"]):
        msg = colored(
            f"Function(s) detected in graded cell number {cell_number} where it shouldn't have a function. The function(s) to be removed or moved to the correct place:\n",
            "red",
        )
    ...
    if (classes is not None) and (not conditions_dict["has_classes"]):
        msg = colored(
            f"Class(es) detected in a graded cell number {cell_number} where it shouldn't have a class. The class(es) to be removed or moved to the correct place:\n",
            "red",
        )
    ...
    if (imports is not None) and conditions_dict["has_imports"]:
        ...
        if (condition_modules) != (got_modules):
            msg = colored(
                f"Incorrect imports. Autograder may fail. Make sure you haven't added or removed imports from graded cell number {cell_number}.\n",
                "red",
            )
        ...
        for module in condition_modules:
            condition_submodules = list(
                set(
                    [
                        x["submodules"]
                        for x in conditions_dict["imports"]
                        if x["module"] == module
                    ][0]
                )
            )
            got_submodules = list(
                set([x["submodules"] for x in imports if x["module"] == module][0])
            )
            condition_submodules.sort()
            got_submodules.sort()
            if condition_submodules != got_submodules:
                msg = colored(
                    f"Incorrect submodules/functions imported from module {module} in cell number {cell_number}. Make sure you haven't added or removed any function/submodule in module {module}, otherwise autograder is likely to fail.\n",
                    "red",
                )
                msg += (
                    f"Expected: module: {module}, submodules: {condition_submodules}\n"
                )
    ...
    if (functions is not None) and conditions_dict["has_functions"]:
        condition_functions = list(
            set([x["function"] for x in conditions_dict["functions"]])
        )
        got_functions = list(set([x["function"] for x in functions]))
        if (condition_functions != got_functions) and (
            len(condition_functions) < len(got_functions)
        ):
            msg = colored(
                f"You added more functions in graded cell {cell_number}. Autograder will likely fail.\n",
                "red",
            )
        ...
        if (condition_functions != got_functions) and (
            ...
            msg = colored(
                f"You removed some functions in graded cell {cell_number}. Check if you haven't added them in another cell. Autograder will likely fail.\n",
                "red",
            )
            removed_functions = [
                x for x in condition_functions if x not in got_functions
            ]
        ...
        if (condition_functions != got_functions) and (
            ...
            msg = colored(
                f"Different set of functions than expected in graded cell number {cell_number}. Make sure you haven't moved any function in the graded cell to another one.",
                "red",
            )
        ...
        for function in condition_functions:
            condition_args = [
                x["args"]["arg_names"]
                for x in conditions_dict["functions"]
                if x["function"] == function
            ][0]
            got_args = [
                x["args"]["arg_names"] for x in functions if x["function"] == function
            ][0]
            if condition_args != got_args:
                msg = colored(
                    f"Arguments in function {function} in graded cell number {cell_number} have been changed. Autograder will likely fail.\n",
                    "red",
                )
                ...
                return False, msg
            if [
                x["args"]["has_non_constant_args"]
                for x in functions
                if x["function"] == function
            ][0]:
                msg = colored(
                    f"Function {function} has non constant args (arguments that points to an object in the jupyter notebook) in graded cell number {cell_number}. Autograder will likely fail.\n",
                    "red",
                )
                return False, msg
    ...
    if (classes is not None) and conditions_dict["has_classes"]:
        ...
        if condition_classes != got_classes:
            msg = colored(
                f"There are extra or missing classes in cell number {cell_number}. Autograder will likely fail.\n",
                "r",
            )
        ...
        for class_ in condition_classes:
            condition_methods = [
                x["methods"] for x in conditions_dict["classes"] if x["name"] == class_
            ][0]
            got_methods = [x["methods"] for x in classes if x["name"] == class_][0]
            condition_function_names = list(
                set([x["function"] for x in condition_methods])
            )
            got_function_names = list(set([x["function"] for x in got_methods]))
            if condition_function_names != got_function_names:
                msg = colored(
                    f"Incorrect methods for class {class_} in cell number {cell_number}. You should not change any given class method. Autograder will likely fail.\n",
                    "red",
                )
    ...
    if not passed and passed is not None:
        ...
        for i, graded_cell in enumerate(graded_cells):
            ...
            analyzed_cells.append(
                cell_checker(tree, code, i + 1 - GRADED_SHIFT, CONDITIONS_DICT[i])
            )
