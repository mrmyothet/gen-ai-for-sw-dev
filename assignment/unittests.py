        ...
        for i, schema in enumerate(graph_schema):
            ...
            for case in test_cases:
                ...
                p = multiprocessing.Process(
                    target=helper,
                    name="shortest_path",
                    args=(q, graph.shortest_path, case),
                )
                p.start()
...
def test_tsp_small_graph(Graph):
    def g():
        ...
        for i, schema in enumerate(graph_schema):
            ...
            p = multiprocessing.Process(
                target=helper,
                name="tsp_small_graph",
                args=(q, graph.tsp_small_graph, 0),
            )
            p.start()
            time.sleep(time_limit)
            if p.is_alive():
                ...
                t.want = (
                    f"shortest_path method must run in less than {time_limit} seconds"
                )
...
def test_tsp_large_graph(Graph):
    def g():
        ...
        for i, schema in enumerate(graph_schema):
            ...
            p = multiprocessing.Process(
                target=helper,
                name="tsp_large_graph",
                args=(q, graph.tsp_large_graph, 0),
            )
            p.start()
            time.sleep(time_limit)
            if p.is_alive():
...
def test_tsp_medium_graph(Graph):
    def g():
        ...
        for i, schema in enumerate(graph_schema):
            ...
            p = multiprocessing.Process(
                target=helper,
                name="tsp_medium_graph",
                args=(q, graph.tsp_medium_graph, 0),
            )
            p.start()
            time.sleep(time_limit)
            if p.is_alive():
                ...
                t.want = (
                    f"{function_name} method must run in less than {time_limit} seconds"
                )
                t.got = f"Time execution exceeded {time_limit} seconds"
            else:
                p.join()
